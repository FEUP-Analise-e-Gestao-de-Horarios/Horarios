...
\subsection{Metodologia utilizada}
    ...
    \\
    A qualidade do código foi assegurada de forma automática em todos os \emph{pull requests} através de \emph{pipelines} de integração contínua (GitHub Actions), que executam \emph{linting}, formatação, verificação estática de tipos e testes - ruff e django-stubs no \emph{backend}; ESLint, Prettier, TypeScript e Vitest no \emph{frontend} - complementadas por \emph{hooks} de \texttt{pre-commit} locais. A atualização de dependências foi automatizada com o Dependabot, em ciclo semanal, garantindo que a aplicação se manteve alinhada com as versões mais recentes das bibliotecas utilizadas. Os ambientes de desenvolvimento e produção foram uniformizados com Docker e \texttt{docker-compose}, orquestrados por um \texttt{Makefile}, e a gestão de dependências Python migrou de pipenv para uv. Para permitir desenvolver e testar a ingestão sem depender do sítio institucional, foi criado um \emph{mirror} local das páginas de horários.
...
\subsection{Intervenientes, papéis e responsabilidades}
    ...
    \\\\
    %% TODO: validar/ajustar a distribuição de responsabilidades — está inferida do histórico do repositório e pode não refletir todo o trabalho (ex.: quem desenvolveu exportador, conflitos e aulas em paralelo nas branches respetivas).
...
\subsection{Arquitetura e tecnologias}
    ...
    \\\\
    \textbf{Ingestão.} A ingestão é um pacote Python independente das \emph{apps} Django, composto por um \emph{scraper} (\texttt{requests} + BeautifulSoup) e por \emph{parsers} dedicados a cada tipo de página (menus de curso, páginas de turma, páginas de docente, blocos de indisponibilidade e salas), que validam os dados extraídos com esquemas Pydantic antes de os persistirem através dos DAOs.
    \\\\
    ...
    %% \begin{center}
        %%\imageplaceholder{Inserir diagrama de componentes: SPA React $\leftrightarrow$ API Django (\texttt{/api/}); backend com apps (login, projects, core), pacote de ingestão, meta-BD Django ORM e BDs por projeto (general/initial) via SQLAlchemy/DAOs; fonte externa SIGARRA}{5cm}
        %%\captionof{figure}{Arquitetura geral da aplicação.}
        %%\label{fig:arquitetura}
    %%\end{center}
    %% TODO: elaborar o diagrama (o ficheiro docs/db_schema.sql e docs/backend.md do repositório podem servir de base); considerar também um diagrama do modelo de dados por projeto.
    %% TODO: quantificar (tempo de ingestão antes/depois) — reforça a secção de validação e os critérios de avaliação.
...
%%  - o que é ingerido (cursos, anos, UCs, turmas, sessões, docentes,
%%    salas, red blocks) e de onde (páginas do SIGARRA);
%%  - as fases do pipeline (scraping -> parsing -> validação Pydantic ->
%%    persistência via DAOs -> snapshot da BD inicial);
%%  - as otimizações de desempenho (inserções em lote, caches de
%%    consultas, controlo de flush) e OS NÚMEROS antes/depois;
%%  - o mirror local usado em desenvolvimento.

\begin{center}
  \imageplaceholder{Inserir diagrama das fases do pipeline de ingestão}{3.4cm}
  \captionof{figure}{Pipeline de ingestão de horários.}
  \label{fig:ingestion}
\end{center}
...
%%  - o que são aulas em paralelo e porque precisam de confirmação manual;
%%  - como os candidatos são detetados (componentes ligados de um grafo de
%%    sobreposição de semanas entre blocos com o mesmo slot);
%%  - a interface: grafos de força interativos (arrastáveis, colapsáveis),
%%    intervalo de semanas em cada nó, semanas de colisão nas arestas,
%%    seleção e confirmação de grupos, indicador de grupos por guardar;
%%  - o que acontece após confirmar (persistência e estado do projeto).

\begin{center}
  \imageplaceholder{Inserir captura de ecrã da página de aulas em paralelo com um grafo de candidatos}{3.4cm}
...
\subsection{Validação}
    ...
    \\\\
    \textbf{Testes automatizados.} A lógica não trivial do editor de horários - colocação de eventos na grelha, filtros, sincronização com o URL, paletas de cores - está isolada em funções puras cobertas por testes unitários (Vitest), executados em todos os \emph{pull requests} na integração contínua. No \emph{backend}, as regras de deteção de conflitos e a persistência de etiquetas são igualmente cobertas por testes dedicados.
%% TODO: indicar o número de testes/ficheiros de teste e, se possível, cobertura aproximada.
    \\\\
    \textbf{Verificação estática contínua.} Todos os \emph{pull requests} passam por \emph{linting}, formatação e verificação de tipos em ambos os módulos, complementados por revisão de código por outro elemento da equipa. Este processo travou regressões de forma recorrente antes de chegarem à \emph{branch} de integração.
    \\\\
    \textbf{Validação da ingestão.} A correção do pipeline de ingestão foi verificada por comparação dos dados persistidos com as páginas reais do SIGARRA, usando o \emph{mirror} local como fonte controlada e reprodutível. Casos-limite identificados durante o desenvolvimento - sessões partilhadas por várias UCs, sessões parciais no semestre, tipologias de aula menos comuns - foram corrigidos e mantidos sob verificação.
%% TODO: acrescentar os números de desempenho da ingestão (tempo antes/depois, dimensão do curso usado como referência).
    \\\\
...
\subsection{Resultados alcançados}
    ...
    O processo de ingestão foi reescrito sobre um novo modelo de dados e otimizado com inserções em lote e \emph{caches} de consultas, reduzindo significativamente o tempo de processamento
%% TODO: substituir por números concretos (de X min para Y s/min)
    sem perda de integridade dos dados.
    ...
    \\\\
    Em paralelo, a base de código ficou muito mais fácil de manter: o código legado foi integralmente substituído por uma arquitetura API/SPA com tipagem de ponta a ponta, testes automáticos, integração contínua, ambientes reprodutíveis e documentação de arquitetura - num total de 662 ficheiros alterados e mais de 230 \emph{pull requests} revistos face à versão herdada. Este investimento beneficia diretamente as equipas das edições futuras do Projeto Integrador.
    \\\\
%% TODO: A PREENCHER PELA EQUIPA — contribuições individuais,qualitativas e quantitativas (percentagem), acordadas entre todos.
    As contribuições individuais distribuíram-se da seguinte forma:
    \textbf{Ana Silva} - \ldots\ (XX\,\%);
    \textbf{Cláudio Meireles} - \ldots\ (XX\,\%);
    \textbf{Diogo Faria} - \ldots\ (XX\,\%);
    \textbf{Pedro Coelho} - \ldots\ (XX\,\%).
...
\subsection{Trabalho futuro} 
    ...
    O passo mais imediato é consolidar o que existe: alargar a cobertura de testes automáticos do \emph{backend} (hoje concentrada na deteção de conflitos) ao pipeline de ingestão e aos restantes \emph{endpoints}, e validar a ferramenta em utilização real com membros das comissões de horários, cujo retorno deverá orientar as prioridades seguintes.
    \\\\
