...
from .db import insert_aula, insert_cursos, insert_turma, insert_ucs, pre_inserir_blocos_vermelhos
from .directories import createDir
from .models import Aula
from .utils import are_weeks_overlapped, get_dia_from_index, get_index, max_date, min_date, table_to_matrix
...
class Parser:
    ...
    def _teardown_success(self) -> None:
        shutil.copy2(self.path + '/general_database.db', self.path + '/initial_database.db')
    ...
    def _parse_horario(self, req: requests.Response, curso_or_uc: str, parsingTurma: bool, lista_de_aulas: set[Aula]) -> None:
        """
            ...
            with open("parser/Salas.txt", "r") as file:
                ...
                for line in file:
                    if sala in line:
                        ...
                        self.cursor.execute(stmt, (sala, tipo, capacidade, tamanhoComp,))
                        self.conn.commit()

                if not alreadyInserted:
                    stmt = '''INSERT INTO salas(numero, tipo, capacidade, tamanhoComp) VALUES (?, ?, ?, ?)'''
                    self.cursor.execute(stmt, (sala, "Desconhecido", "Desconhecido", "Desconhecido",))
                    self.conn.commit()
    ...
    def _cleanup_aulas(self) -> None:
        ...
        for aula in aulas:
            ...
            self.cursor.execute(stmtTest, (dia, hora, duracao, isTeorica, docente, uc, turma))
            results = self.cursor.fetchall()

            while len(results) > 1 and any(
                are_weeks_overlapped(results[i][5], results[i][6], results[j][5], results[j][6])
                ...
                if are_weeks_overlapped(si1, sf1, si2, sf2):
                    ...
                    for table in ['aulaDocente', 'aulaUC', 'aulaSala', 'aulaTurmas']:
                        self.cursor.execute(f'DELETE FROM {table} WHERE idAula=?', (idAula2,))
