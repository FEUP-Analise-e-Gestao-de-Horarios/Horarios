...
from src.getHorariosFromDB.movementFunctions import (
    addDocente,
    addSala,
    addTurma,
    changeUC,
...
def getProjetosListAux(request, userId):
    ...
    for i in courses:
        if i["member_groups"] not in merge_courses:
    ...
    for project in projects:
        if project[3] in ids:
            ...
            person == project[0]
            or person == project[2]
            or (project[1] in merge_courses and project[1] != None and merge_courses != None)
...
def areSemanasCompatible(siAula, sfAula, siSelected, sfSelected):
    return (
        (siAula == siSelected and sfAula == sfSelected)
        or (siAula < siSelected and sfAula == sfSelected)
        or (siAula == siSelected and sfAula > sfSelected)
    )
            ...
            for k in people:
                if k[0] != request.user.pk:
                    if k[1] == i[1] or request.user.is_staff:
                        ...
                        if not (
                            User.objects.get(pk=k[0]) in temp1 or User.objects.get(pk=k[0]) in temp
                        ):
    ...
    try:
        ...
        for sala in salas:
            ...
            for row in aulasSalaRows:
                ...
                if (
                    semanaInterval == None
                    or semanaInterval == "Semanas"
                    or areSemanasCompatible(semanaInicial, semanaFinal, start_date, end_date)
                ):
                    aulasSala.append(
                        Aula(
                            row["id"],
        ...
        docentes = [
            Docente(row["numeroMecanografico"], row["nome"], row["abreviacao"])
            for row in docentesRows
        ]
            ...
            for row in aulasUCRows:
                ...
                if (
                    ...
                    or (
                        start_date
                        and end_date
                        and areSemanasCompatible(semanaInicial, semanaFinal, start_date, end_date)
                    )
                ):
                    aulasUC.append(
                        Aula(
        ...
        for docente in docentesAno:
            ...
            docenteBloco = [
                Bloco(row["id"], row["hora"], row["diaSemana"]) for row in docenteBlocoRows
            ]
        ...
        logger.error(f"Error in fillPageForCursoAno: {e!s}", exc_info=True)
...
def uc_view(request: HttpRequest, projId: int, uc_codigo: str) -> HttpResponse:
    ...
    for aula in aulas:
        ...
        for docente_id in aula_docentes:
            cursor.execute(
                """SELECT numeroMecanografico, nome, abreviacao 
                            FROM docentes 
...
def get_uc_list(request):
    ...
    try:
        ...
        logger.error(f"Error in get_uc_list: {e!s}", exc_info=True)
        return JsonResponse({"error": str(e)}, status=500)
...
def swap_teachers(request, projId):
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                """SELECT numeroMecanografico, abreviacao 
                            FROM docentes 
            ...
            cursor.execute(
                """INSERT INTO aulaDocente (idAula, idDocente)
                            VALUES (?, ?)""",
                (aula1["id"], teacher2),
            )
    ...
    try:
        ...
        try:
            ...
            if change_type == "move":
                ...
                cursor.execute(
                    "SELECT numeroMecanografico, abreviacao FROM docentes WHERE numeroMecanografico IN (?, ?)",
                    (teacher1, teacher2),
                )
                teachers = {str(row[0]): row[1] for row in cursor.fetchall()}
                ...
                # Remove existing assignments
                cursor.execute(
                    "DELETE FROM aulaDocente WHERE idAula = ? AND idDocente = ?",
                    (aula1["id"], teacher1),
                )
                cursor.execute(
                    "DELETE FROM aulaDocente WHERE idAula = ? AND idDocente = ?",
                ...
                # Create new assignments
                cursor.execute(
                    "INSERT INTO aulaDocente (idAula, idDocente) VALUES (?, ?)",
                    (aula1["id"], teacher2),
                )
                cursor.execute(
                    "INSERT INTO aulaDocente (idAula, idDocente) VALUES (?, ?)",
            ...
            conn.rollback()
            return JsonResponse({"success": False, "error": f"Database error: {e!s}"}, status=500)
        finally:
            conn.close()
...
def getAulaSimultaneasParalelas(request):
    try:
        ...
        if aulaId_1 is not None:
            cursor.execute(
                "SELECT COUNT(*) FROM turmasSimultaneas WHERE aula1 = ? OR aula2 = ?",
                [aulaId_1, aulaId_1],
            )
        ...
        if aulaId_1 is not None:
            cursor.execute(
                "SELECT COUNT(*) FROM aulasSimultaneas WHERE aula1 = ? OR aula2 = ?",
                [aulaId_1, aulaId_1],
            )
        ...
        if (aulaId_1 is not None) and (aulaId_2 is not None):
            grupo_paralelo_id = {
                aula["id"] for aula in grupo_paralelo
            }  # grupo_paralelo but it's just a list of ids
            grupo_simultanea_id = {
                aula["id"] for aula in grupo_simultanea
            }  # grupo_simultanea but it's just a list of ids
...
def editDocentes(request, projId):
    ...
    return render(
        request,
        "editTurnos/editDocentes.html",
        {"docentes": docentesList, "projetos": projetos, "projId": projId, "is_edit_turnos": True},
    )
...
def editDocentesMakeChange(request, projId):
    ...
    conn.commit()
    return JsonResponse(
        {"idDocente": idDocente, "nomeDocente": nomeDocente, "siglaDocente": siglaDocente},
        status=200,
    )
...
def switch_number_to_day(number_string):
    ...
    return switch_dict.get(number_string)
...
def get_aula_info(projId, aulaId):
    ...
    try:
        ...
        row_dict = {
            ...
            "idDocente": aula_data["docentes_ids"].split(",")[0]
        ...
        data.update(
            {
                ...
                "docentesIds": [str(d) for d in aula_data["docentes_ids"].split(",")]
                if aula_data["docentes_ids"]
                else [],
