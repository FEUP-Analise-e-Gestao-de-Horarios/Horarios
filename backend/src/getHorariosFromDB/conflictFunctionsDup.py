...
def checkIfDocenteConflict(ProjectNumber, day, hour, idAula):
    ...
    for docente in manchaDocentes:
        ...
        if hourDay in manchaDocentes[docente]:
            listaAulas = aux.getAulaFromDocenteAndTime(
                ProjectNumber,
                int(hour),
                day,
                docente[0],
            ...
            if listaAulas is not None:
                if len(listaAulas) >= 1:  # mais de uma aula: conflitos
                    if listaAulas[0] != -1:
                        everyConflict = []
                        for i in listaAulas:
    ...
    if len(listaConflitos) != 0:
        ...
        for docente in listaConflitos:
            mensagemErro = (
                "Docente "
                + aux.getAbreviacaoFromMecanografico(ProjectNumber, docente)
                + " está ocupado"
            )
            ...
            for conflito in listaConflitos[docente]:
                ...
                graph_controller.add_edge_to_graph(
                    ProjectNumber,
                    conflito[0],
                    conflito[1],
                    mensagemErro,
...
def checkIfTurmaConflict(ProjectNumber, day, hour, idAula):
    ...
    for turma in manchaTurma:
        ...
        if hourDay in manchaTurma[turma]:
            listaAulas = aux.getAulaFromTurmaAndTime(
                ProjectNumber,
                int(hour),
                day,
                turma[0],
...
def organizeInformation(ProjectNumber, conflicts):
    ...
    path = "Project" + str(ProjectNumber)
    file_path = "./database/" + path + "/conflicts_text_dup.txt"
    with open(file_path, "w") as file:
        for aulas in conflicts:
            ...
            if informationAula1 == informationAula2:
                resultString = formatForSameAula.format(
                    informationAula1[2],
                    informationAula1[1],
                    converter_horario(informationAula1[0]),
                    informationAula1[3],
                    conflicts[aulas],
                )
            else:
                resultString = formatting.format(
                    informationAula1[2],
                    informationAula1[1],
                    converter_horario(informationAula1[0]),
                    informationAula1[3],
                    informationAula2[2],
                    informationAula2[1],
                    converter_horario(informationAula2[0]),
                    informationAula2[3],
                    conflicts[aulas],
                )
