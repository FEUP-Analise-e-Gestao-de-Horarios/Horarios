...
def check_conflicts(aula1: AulaInfo, aula2: AulaInfo):
    ...
    if hora_inicio_a2 < hora_fim_a1 and hora_fim_a2 > hora_inicio_a1 and aula1.dia == aula2.dia:
        if (
            aula1.salas_ids.intersection(aula2.salas_ids)
            or aula1.turmas_ids.intersection(aula2.turmas_ids)
            or aula1.docentes_ids.intersection(aula2.docentes_ids)
        ):
            return True
    return False
...
def check_sala_conflicts(
    cursor,
    salas_ids,
    dia_semana,
    hora_inicio,
    hora_fim,
    exclude_aula_id=None,
):
    conflicts = []
    ...
    for docente in docentes_ids:
        ...
        for aula_id, aula_hora, aula_duracao, uc_nome, docente_nome in cursor.fetchall():
            ...
            if hora_inicio < aula_end_min and hora_fim > aula_hora_min:
                ...
                print(
                    f"Checking docente {docente} on aula -> {exclude_aula_id}, {dia_semana} at {hora_inicio // 60}-{hora_fim // 60} vs aula {aula_id} at {dia_semana} {aula_hora_str}-{aula_end_min // 60}:{aula_end_min % 60}",
                )

    return conflicts


def check_turma_conflicts(
    cursor,
    turmas_ids,
    dia_semana,
    hora_inicio,
...
def check_aula_change_conflicts(change: AulaChange, project_number, mode):
    ...
    try:
        ...
        conflicts.extend(
            check_docente_conflicts(
                cursor,
                new_aula.docentes_ids,
                dia_semana,
                hora_inicio,
                hora_fim,
                aula_id,
        ...
        conflicts.append(f"unexpected_error:{e!s}")
    finally:
        if "cursor" in locals():
            cursor.close()
        if "connection" in locals():
